do
    ...
    --local tempFitnessTensor = torch.Tensor(offspring)
		--local fitnesses = tempFitnessTensor:map(self.offspring, function(k, v) return v.fitness or 0 end)
    local fitnesses = {}
    for ind = 1, #self.offspring do
      print('Offspring ' .. ind .. ' has fitness ' .. self.offspring[ind].fitness)
      fitnesses[ind] = self.offspring[ind].fitness
    end
    
    --local fitnesses = torch.Tensor(1)
    --fitnesses.map(self.offspring, function(k, v) return v.fitness or 0 end)
    ...
    --local tempGenomesTensor = torch.Tensor(self.offspring)
		--local genomes = tempGenomesTensor:map(self.offspring, function(k, v) return v.genome:totable() end)
    local genomes = {}
    for ind = 1, #self.offspring do
      genomes[ind] = self.offspring[ind].genome:totable()
    end
