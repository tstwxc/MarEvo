do
    ...
    --print('Statecode is ' .. stateCode)

		return states[stateCode]
	end
  
  function Inputs.getMarioPowerupState()
		-- 0x0756	Player's Powerup state
		local states = { 
			'Small',
			'Big',
			'Fiery'
		}
		local stateCode = memory.readbyte(0x756)
    --print('Statecode is ' .. stateCode)

		return states[stateCode + 1]
	end
  
  function Inputs.getCoins()
      local num_coins = memory.readbyte(0x75E)
	...
	function Inputs.getDistances(mario, sprites)
		local distances = {}
		for i = 1, #sprites do
			table.insert(distances, Helpers.distance(mario, sprites[i]))
		end
